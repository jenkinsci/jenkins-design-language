...
storiesOf('Table', module)
        ...
        return (
            <div>
                <Table values={usersCopy}>
                    <Table.Col id header="Name" expand render={(user: any) => <div>{user.name}</div>} />
                    <Table.Col header="Age"render={(user: any) => <div>{user.age}</div>} />
                </Table>
                <button onClick={() => usersCopy[0].age = '99'}>This won't update the table</button>
                <button onClick={() => usersCopy[0].name = 'asdf'}>This will update the table</button>
            </div>
        );
    })
    .add('supports key by id={fn}', () => {
        ...
        return (
            <Table values={usersCopy}>
                <Table.Col id={u => u.name} header="Name" expand render={(user: any) => <div>{user.name}</div>} />
                <Table.Col header="Age"render={(user: any) => <div>{user.age}</div>} />
            </Table>
        );
    })
