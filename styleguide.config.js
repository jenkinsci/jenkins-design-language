...
class BeautifyPlugin {
    constructor(config = {}) {
        this.webpack4 = webpack.version && /.*4(\.\d+){0,2}/gi.test(webpack.version);
        this.options = {
    ...
    beautify(compilation) {
        ...
        Object.keys(compilation.assets).forEach(k => {
            ...
            if (/.*\.js/.test(k) && asset.source) {
                ...
                compilation.assets[k] = new RawSource(beautify(source, this.options));
                ...
                compilation.assets[k] = new RawSource(beautify.html(source, this.options));
            } else if (/.*\.css/.test(k) && asset.source) {
                compilation.assets[k] = new RawSource(beautify.css(asset.source(), this.options));
            }
        });
    }
